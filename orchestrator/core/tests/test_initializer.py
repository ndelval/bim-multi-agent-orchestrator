...
class TestOrchestratorInitializer:
    ...
    def test_initialize_memory_none(self, initializer):
        """Test memory initialization when config.memory is None."""
        # OrchestratorConfig.__post_init__ auto-creates MemoryConfig,
        # so we must explicitly set memory to None to test this path.
        initializer.config.memory = None
    ...
    @patch("orchestrator.core.initializer.GraphFactory")
    def test_create_langgraph_system_without_tasks(
        self, mock_graph_factory, basic_config
    ):
