...
class ResearchTaskTemplate(BaseTaskTemplate):
    ...
    def create_task(self, config: TaskConfig, agent: Agent, **kwargs) -> Task:
        ...
        try:
            ...
            raise TaskExecutionError(f"Failed to create research task '{config.name}': {str(e)}")
...
class PlanningTaskTemplate(BaseTaskTemplate):
    ...
    def create_task(self, config: TaskConfig, agent: Agent, **kwargs) -> Task:
        ...
        try:
            ...
            raise TaskExecutionError(f"Failed to create planning task '{config.name}': {str(e)}")
...
class ImplementationTaskTemplate(BaseTaskTemplate):
    ...
    def create_task(self, config: TaskConfig, agent: Agent, **kwargs) -> Task:
        ...
        try:
            ...
            raise TaskExecutionError(f"Failed to create implementation task '{config.name}': {str(e)}")
...
class TestingTaskTemplate(BaseTaskTemplate):
    ...
    def create_task(self, config: TaskConfig, agent: Agent, **kwargs) -> Task:
        ...
        try:
            ...
            raise TaskExecutionError(f"Failed to create testing task '{config.name}': {str(e)}")
...
class ReviewTaskTemplate(BaseTaskTemplate):
    ...
    def create_task(self, config: TaskConfig, agent: Agent, **kwargs) -> Task:
        ...
        try:
            ...
            raise TaskExecutionError(f"Failed to create review task '{config.name}': {str(e)}")
...
class DocumentationTaskTemplate(BaseTaskTemplate):
    ...
    def create_task(self, config: TaskConfig, agent: Agent, **kwargs) -> Task:
        ...
        try:
            ...
            raise TaskExecutionError(f"Failed to create documentation task '{config.name}': {str(e)}")
...
class TaskFactory:
    ...
    def create_tasks_from_configs(
        ...
        for config in sorted_configs:
            ...
            if agent is None:
                raise TaskExecutionError(f"Agent '{config.agent_name}' not found for task '{config.name}'")
        ...
        for config in configs:
            ...
            next_tasks = [task_map[name] for name in config.next_tasks if name in task_map]
            if next_tasks:
    ...
    def get_execution_order(self, configs: List[TaskConfig]) -> List[List[str]]:
        ...
        while remaining:
            ...
            if not current_level:
                raise DependencyError("Cannot resolve task dependencies - possible circular reference")
