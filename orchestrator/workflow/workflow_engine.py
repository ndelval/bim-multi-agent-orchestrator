...
logger.info("WorkflowEngine initialized with LangChain backend")
...
class WorkflowEngine:
            ...
            for dep_name in execution.dependencies:
                if dep_name not in all_task_names:
                    raise DependencyError(
                        f"Task '{task_name}' depends on non-existent task '{dep_name}'"
                    )

            for next_task in execution.dependents:
                if next_task not in all_task_names:
                    raise DependencyError(
                        f"Task '{task_name}' references non-existent next task '{next_task}'"
                    )
            ...
            for task_name in level:
                try:
                    ...
                    if not self._should_continue_on_failure(task_name):
                        raise WorkflowError(
                            f"Workflow stopped due to critical task failure: {task_name}"
                        )
        ...
        for level in execution_levels:
            if len(level) == 1:
                ...
                semaphore = asyncio.Semaphore(
                    min(self.max_concurrent_tasks, len(level))
                )
    ...
    async def _execute_task(self, task_name: str) -> Any:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                # Execute the task -
                # LangChain task executionx
                context_data = {
                    "task_name": task_name,
                    "config": execution.config,
                    "dependencies": [
                        self.executions[dep].result
                        for dep in execution.dependencies
                ...
                if self.timeout:
                    ...
                    result = await asyncio.to_thread(
                        execution.task.execute, context_data
                    )
                ...
                logger.info(
                    f"Task completed: {task_name} in {execution.end_time - execution.start_time:.2f}s"
                )
                return result
                ...
                execution.error = e
                logger.warning(
                    f"Task {task_name} attempt {attempt + 1} failed: {str(e)}"
                )
                ...
                if attempt < self.max_retries:
                    ...
                    raise TaskExecutionError(
                        f"Task {task_name} failed after {self.max_retries + 1} attempts: {str(e)}"
                    )
    ...
    def _should_continue_on_failure(self, task_name: str) -> bool:
        ...
        for dependent_name in execution.dependents:
            if dependent_name in self.executions:
                ...
                if (
                    dependent_execution.config.task_type == "decision"
                    or not dependent_execution.dependents
                ):
                    return False
    ...
    def get_workflow_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "pending_tasks": len(
                [e for e in self.executions.values() if e.status == TaskStatus.PENDING]
            ),
            "ready_tasks": len(
                [e for e in self.executions.values() if e.status == TaskStatus.READY]
            ),
            ...
            "metrics": {
                ...
                "current_time": (
                    time.time() if self.is_running else self.metrics.end_time
                ),
                "duration": (
                    (time.time() - self.metrics.start_time)
                    if self.is_running and self.metrics.start_time
                    else self.metrics.total_duration
                ),
            },
        }
    ...
    def export_execution_graph(self) -> Dict[str, Any]:
        ...
        for task_name, execution in self.executions.items():
            ...
            for dependent in execution.dependents:
                edges.append({"from": task_name, "to": dependent, "type": "dependency"})
